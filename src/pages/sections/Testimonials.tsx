...
export default function Testimonials() {
  return (
    <section id="testimonials" className="py-24 px-4 bg-gradient-to-b from-slate-900 to-slate-800 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full opacity-10">
        <div className="absolute top-1/4 left-10 w-96 h-96 bg-blue-500 rounded-full blur-3xl" />
        <div className="absolute bottom-1/4 right-10 w-96 h-96 bg-sky-400 rounded-full blur-3xl" />
      </div>

      <div className="container mx-auto max-w-7xl relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-4 text-white font-serif drop-shadow-lg">What Our Customers Say</h2>
          <p className="text-lg text-sky-200 font-sans">Trusted by thousands across Pune and nearby areas</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((t, i) => (
            <motion.div key={t.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: i * 0.1 }} viewport={{ once: true }}>
              <Card className="h-full bg-slate-800/50 backdrop-blur-sm hover:bg-slate-700/50 shadow-lg hover:shadow-xl hover:shadow-blue-500/10 hover:-translate-y-2 transition-all duration-300 group border-slate-600">
                <CardContent className="p-6">
                  <div className="flex items-center gap-4 mb-4">
                    <div>
                      <h4 className="font-semibold text-sky-100 font-serif group-hover:text-sky-200 transition-colors">{t.name}</h4>
                      <div className="flex gap-1">{Array.from({ length: t.rating }).map((_, i) => <Star key={i} className="w-4 h-4 fill-blue-400 text-blue-400" />)}</div>
                    </div>
                  </div>
                  <p className="text-slate-300 italic leading-relaxed group-hover:text-white transition-colors font-sans">"{t.text}"</p>
                </CardContent>
              </Card>
            </motion.div>
          ))}
